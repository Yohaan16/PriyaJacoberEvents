...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    for (float i, a; i++ < 24.0; )
    {
        // denom: max(a = i*i/240.0 - length(p)/r.y, -a*1.8) + 6.0/r.y
        a = (i * i) / 240.0 - length(p) / r.y;
        float denom = max(a, -a * 1.8) + 6.0 / r.y;
        ...
        // o += .09/denom * sm * (1.4 + sin(a + i + vec4(0,2,4,0)))
        o += 0.09 / denom * sm * (1.4 + sin(a + i + vec4(0.0, 2.0, 4.0, 0.0)));
    }
...
function ShaderCanvas({
  ...
  useEffect(() => {
    ...
    const rawGl = canvas.getContext("webgl2", { premultipliedAlpha: false });
    if (!rawGl) return;
